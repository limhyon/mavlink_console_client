...
double								sT;						// Sampling time of controller (Automatically calculated, do not touch, change frequency)
bool		flag = true;
...
void retrieve_VICON( void )
{
	...
	for( unsigned int SubjectIndex = 0 ; SubjectIndex < SubjectCount ; ++SubjectIndex )
	{
		...
		for( unsigned int MarkerIndex = 0 ; MarkerIndex < MarkerCount ; ++MarkerIndex )
		{
			...
			if (SubjectIndex == 0 && MarkerIndex == 3)
			{
			...
			else if (SubjectIndex == 0 && MarkerIndex == 2)
			{
			...
			else if (SubjectIndex == 0 && MarkerIndex == 0)
			{
			...
			else if (SubjectIndex == 0 && MarkerIndex == 1)
			{
		...
		if( SubjectIndex == 0 )
		{
			...
			// Calculate rate values : Problematic.
